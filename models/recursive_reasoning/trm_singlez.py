...
@dataclass
class SingleZLatentState:
    """Single z variant - only carries latent reasoning, no separate answer"""

    z: torch.Tensor


@dataclass
class TinyRecursiveModelSingleZState:
    """State for TinyRecursiveModel single-z variant"""

    latent: SingleZLatentState
...
class TinyRecursiveModelSingleZConfig(BaseModel):
    batch_size: int
    seq_len: int
    ...
    y_cycles: int  # Number of y update cycles (was H_cycles in HRM)
    z_cycles: int  # Number of z recursion steps per y cycle (was L_cycles in HRM)

    H_layers: int = 0  # Deprecated, ignored
...
class TinyRecursiveModelSingleZBlock(nn.Module):
    def __init__(self, config: TinyRecursiveModelSingleZConfig) -> None:
        super().__init__()
...
class TinyRecursiveModelSingleZReasoningModule(nn.Module):
    def __init__(self, layers: List[TinyRecursiveModelSingleZBlock]):
        super().__init__()
...
class TinyRecursiveModelSingleZ_Inner(nn.Module):
    def __init__(self, config: TinyRecursiveModelSingleZConfig) -> None:
        super().__init__()
        self.config = config
    ...
    def forward(
        ...
        with torch.no_grad():
            for _y_cycle in range(self.config.y_cycles - 1):
                # z_cycles latent recursion steps: update z given (x, z)
                for _z_cycle in range(self.config.z_cycles):
                    z = self.net(z + input_embeddings, **seq_info)
                # Additional refinement step
                z = self.net(z, **seq_info)
        ...
        # Output: decode z directly to vocabulary logits
        new_latent = SingleZLatentState(
            z=z.detach()
        )  # Detach for next supervision step
        output = self.lm_head(z)[:, self.puzzle_emb_len :]
        q_logits = self.q_head(z[:, 0]).to(torch.float32)  # Q-head uses first position
        return new_latent, output, (q_logits[..., 0], q_logits[..., 1])


class TinyRecursiveModelSingleZ(nn.Module):
    """Tiny Recursive Model with single z variant - Adaptive Computational Time (ACT)."""
    ...
    def __init__(self, config_dict: dict):
        ...
        self.config = TinyRecursiveModelSingleZConfig(**config_dict)
        self.inner = TinyRecursiveModelSingleZ_Inner(self.config)
    ...
    def forward(
        self,
        carry: TinyRecursiveModelSingleZState,
        batch: Dict[str, torch.Tensor],
    ) -> Tuple[TinyRecursiveModelSingleZState, Dict[str, torch.Tensor]]:
        ...
        # Forward inner model
        new_latent, logits, (q_halt_logits, q_continue_logits) = self.inner(
...
TinyRecursiveReasoningModel_ACTV1 = TinyRecursiveModelSingleZ
TinyRecursiveReasoningModel_ACTV1Config = TinyRecursiveModelSingleZConfig
TinyRecursiveReasoningModel_ACTV1Carry = TinyRecursiveModelSingleZState
TinyRecursiveReasoningModel_ACTV1InnerCarry = SingleZLatentState
